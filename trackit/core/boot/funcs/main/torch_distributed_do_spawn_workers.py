...
def spawn_workers(args):
    ...
    torch_run_args = ['--nnodes', str(args.distributed_nnodes),
                      '--nproc_per_node', str(args.distributed_nproc_per_node),
                      '--node_rank', str(args.distributed_node_rank),
                      '--rdzv_endpoint', f'{args.master_address}:{rdzv_port}',
                      '--max_restarts', str(0),
                      '--rdzv_id', args.run_id,
                      '--rdzv_backend', 'static',
                      '--master_addr', args.master_address,
                      '--master_port', str(rdzv_port),
                      train_script_path]

    index_of_arg = 1
    while index_of_arg < len(command_args):
        command_arg = command_args[index_of_arg]
        if command_arg in ('--distributed_nnodes', '--distributed_nproc_per_node',
                           '--run_id', '--distributed_node_rank'):
            index_of_arg += 2
            ...
            index_of_arg += 1
    torch_run_args.extend(['--run_id', args.run_id])
