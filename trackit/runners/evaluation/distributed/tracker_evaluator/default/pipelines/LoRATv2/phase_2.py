...
from .update_criteria import ScoreBasedCriteria
...
class LoRATv2_Phase_2_TrackingPipeline(TrackingPipeline):
    def __init__(self, device: torch.device,
                 curated_template_image_size: Tuple[int, int],
                 curated_search_image_size: Tuple[int, int],  # W, H
                 curated_dynamic_reference_image_size: Tuple[int, int],
                 search_curation_parameter_provider_factory: Callable[[], SiamFCCroppingParameterProvider],
                 dynamic_reference_frame_cropping_area_factor: float,
                 dynamic_reference_frame_update_score_threshold: float,
        ...
        self.curated_dynamic_reference_image_size = curated_dynamic_reference_image_size
        self.dynamic_reference_frame_cropping_area_factor = dynamic_reference_frame_cropping_area_factor
        ...
        self.dynamic_reference_frame_update_criteria = ScoreBasedCriteria('dynamic_reference_frame_update_criteria',
                                                                          dynamic_reference_frame_update_score_threshold)
    ...
    def start(self, evaluator_context: EvaluatorContext, *_):
        ...
        curated_dynamic_reference_image_cache_shape = (max_batch_size, 3, self.curated_dynamic_reference_image_size[1], self.curated_dynamic_reference_image_size[0])
        ...
        self.curated_dynamic_reference_image_cache = torch.empty(curated_dynamic_reference_image_cache_shape, dtype=evaluator_context.dtype,
                                                        device=self.device).fill_(float('nan'))
        ...
        self.dynamic_reference_frame_update_criteria.start()
    ...
    def stop(self, evaluator_context: EvaluatorContext, *_):
        ...
        self.dynamic_reference_frame_update_criteria.stop()
        ...
        del self.curated_dynamic_reference_image_cache
    ...
    def initialize(self, data: TrackerEvalData, model, context: TrackerEvaluationPipeline_Context, raw_model: nn.Module):
        ...
        for task in data.tasks:
            ...
            if task.tracker_do_init_context is not None:
                ...
                self.curated_template_image_cache[num_initializing_sequence, ...] = init_context.input_data[
                    'curated_image']
        ...
        if num_initializing_sequence > 0:
            ...
            model({'action': 'init',
                   ...
                   **kv_cache_params})
            self._update(do_initialization_task_ids, data, model)
    ...
    def track(self, data: TrackerEvalData, model, context: TrackerEvaluationPipeline_Context,
        ...
        for task in data.tasks:
            if task.tracker_do_tracking_context is not None:
                ...
                cropping_parameter_providers.append(
                    self.all_tracking_task_contexts[task.id].curation_parameter_provider)
        ...
        if num_tracking_sequence > 0:
            ...
            do_update_task_ids_indices = self.dynamic_reference_frame_update_criteria(predicted_scores)
            do_update_task_ids = [
                task_ids[index] for index in do_update_task_ids_indices
            ]
            do_update_task_ids = [task_id for task_id in do_update_task_ids if task_id not in task_ids_to_be_finalized]
            if len(do_update_task_ids) > 0:
                self._update(do_update_task_ids, data, model)
    ...
    def _update(self, do_update_task_ids, data: TrackerEvalData, model):
        num_do_update_sequence = 0
        for task in data.tasks:
            if task.id in do_update_task_ids:
                assert task.tracker_do_tracking_context is not None
                search_image = task.tracker_do_tracking_context.input_data['image']
                cropping_parameter_provider = self.all_tracking_task_contexts[task.id].curation_parameter_provider

                template_image_mean = self.all_tracking_template_image_mean_cache.get(task.id)
                curation_parameter = cropping_parameter_provider.get(np.array(self.curated_dynamic_reference_image_cache),
                                                                     self.dynamic_reference_frame_cropping_area_factor)

                _, _, curation_parameter = \
                    apply_siamfc_cropping(search_image.to(torch.float),
                                          np.array(self.curated_dynamic_reference_image_size), curation_parameter,
                                          self.interpolation_mode, self.interpolation_align_corners,
                                          template_image_mean,
                                          out_image=self.curated_dynamic_reference_image_cache[num_do_update_sequence, ...])
                num_do_update_sequence += 1
        assert num_do_update_sequence == len(do_update_task_ids)
        x = self.curated_dynamic_reference_image_cache[: num_do_update_sequence, ...]
        x.div_(255.)
        self.normalization_transform_(x)
        x = x.to(self.dtype)
        model({'action': 'update',
               'index': 1,
               'x': x,
               **self.kv_cache.get_input_params(do_update_task_ids)})
