...
class SPMTrack_EvaluationPipeline(TrackingPipeline):
    ...
    def start(self, context: EvaluatorContext, global_objects: dict):
        ...
        assert context.dtype == torch.float32, "SPMTrack pipeline only supports float32 dtype"
        self.all_tracking_task_local_contexts: Dict[Any, _LocalContext] = {}
        self.all_tracking_template_cache = CacheService(TensorCache(max_num_concurrent, template_shape, self.device))
    ...
    def initialize(self, data: TrackerEvalData, model, context: TrackerEvaluationPipeline_Context, raw_model: nn.Module):
        ...
        for task in data.tasks:
            if task.tracker_do_init_context is not None:
                ...
                if task.id in self.memory_frames.keys():
                    self.memory_frames[task.id].append(init_context.input_data['curated_image'].cpu())
                else:
                    self.memory_frames[task.id] = [init_context.input_data['curated_image'].cpu()]
        ...
        if num_tracking_sequence > 0:
            ...
            for i, task_id in enumerate(task_ids):
                if len(self.memory_frames[task_id]) == 1:
                    model_input_params['z_0'].append(self.memory_frames[task_id][0].to(x.device))
                    model_input_params['z_1'].append(self.memory_frames[task_id][0].to(x.device))
                    model_input_params['z_2'].append(self.memory_frames[task_id][0].to(x.device))
                elif len(self.memory_frames[task_id]) == 2:
                    model_input_params['z_0'].append(self.memory_frames[task_id][0].to(x.device))
                    model_input_params['z_1'].append(self.memory_frames[task_id][1].to(x.device))
                    model_input_params['z_2'].append(self.memory_frames[task_id][1].to(x.device))
            ...
            for index, (task_id, image_size, frame_index) in enumerate(zip(task_ids, x_image_size_list, x_frame_indices)):
                ...
                for task_input_data in data.tasks:
                    if task_input_data.id == task_id:
                        ...
                        template_curation_parameter = get_siamfc_cropping_params(predicted_bounding_box_on_full_search_image, 2.0, np.array(self.template_image_size)) # 2, [196,196]
