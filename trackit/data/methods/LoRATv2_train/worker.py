...
from .temporal_training_tuplet_sampling import SamplingResult_Element, TemporalTrackerTrainingSamplesSampler
from ._types import TemporalTrackerTrainingSample, SOTFrameInfo
from .transform import TemporalTrackerTrain_DataTransform
...
def _prepare_siamese_training_pair(sampler_index: Optional[int],
                                   ...
                                   training_sample_sampler: TemporalTrackerTrainingSamplesSampler,
                                   rng_engine: np.random.Generator, prefetch: bool,
                                   global_job_index: Optional[int] = None, batch_element_index: Optional[int] = None):
    training_samples = training_sample_sampler(sampler_index, rng_engine)

    result = {}
    cache = {}
    for i, z in enumerate(training_samples.templates):
        _decode_with_cache(f'z_{i}', z, datasets, cache, result, rng_engine, prefetch)
    for i, x in enumerate(training_samples.search_regions):
        _decode_with_cache(f'x_{i}', x, datasets, cache, result, rng_engine, prefetch)

    decoded_training_sample = TemporalTrackerTrainingSample(
        tuple(result[f'z_{i}'] for i in range(len(training_samples.templates))),
        tuple(result[f'x_{i}'] for i in range(len(training_samples.search_regions))),
        training_samples.search_region_target_exists
    )
    
    if global_job_index is not None:
        return global_job_index, batch_element_index, decoded_training_sample
    else:
        return decoded_training_sample


class TemporalTrackerTrainingDataWorker(torch.utils.data.Dataset):
    def __init__(self, datasets: Sequence[TrackingDataset],
                 num_samples_per_epoch: int, batch_size: int,
                 training_pair_generator: TemporalTrackerTrainingSamplesSampler,
                 data_transform: TemporalTrackerTrain_DataTransform,
                 num_io_threads: int):
        self.datasets = datasets
        self.num_samples_per_epoch = num_samples_per_epoch
        self.batch_size = batch_size
        self.training_pair_generator = training_pair_generator
    ...
    def __getitems__(self, job_indices: Sequence[int]):
        ...
        if self.num_io_threads > 0:
            ...
            jobs = tuple(self.background_io_threads.submit(_prepare_siamese_training_pair,
                                                        job_index,
                                                           self.datasets, self.training_pair_generator,
...
class TemporalTrackerTrainingDataCollator:
...
class TemporalTrackerTrainingMainProcessLoggingHook(MainProcessDataPipeline):
