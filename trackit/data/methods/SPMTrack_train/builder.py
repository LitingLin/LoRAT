...
from .worker import SPMTrackTrackerTrainingDataWorker, SPMTrackTrackerTrainingDataCollator, SPMTrackTrackerTrainingHostLoggingHook
from .training_tuplet_sampling.builder import build_training_tuplet_sampler
from .transform.builder import build_transform


def build_SPMTrack_train_data_pipeline(data_config: dict, build_context: BuildContext, config: dict, dtype: torch.dtype) -> DataPipeline:
    ...
    training_tuplet_sampler = build_training_tuplet_sampler(datasets, datasets_sampling_weight, sampler,
                                                            data_config['tuplet_sampling'])
    ...
    worker = SPMTrackTrackerTrainingDataWorker(datasets, num_samples_per_epoch, local_batch_size,
                                               training_tuplet_sampler,
                                               transform, num_io_threads)

    dataloader = build_dataloader(worker, local_batch_size, num_workers, build_context, do_shuffle=False,
                                  collate_fn=SPMTrackTrackerTrainingDataCollator(transform_batch_collator))
    ...
    logging_hook = SPMTrackTrackerTrainingHostLoggingHook(num_io_threads)
