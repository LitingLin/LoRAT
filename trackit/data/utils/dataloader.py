...
import torch
from torch.utils.data import DataLoader, IterableDataset, DistributedSampler, Dataset
...
def build_dataloader(dataset: Dataset, batch_size: Optional[int], num_workers: int,
    ...
    worker_info_initializer = WorkerInfoInitializer(get_rank(), build_context.seed, num_workers if num_workers > 0 else 1, get_world_size(), num_workers != 0)
    worker_init_fn = worker_info_initializer.worker_init_fn if num_workers > 0 else None
    ...
    if isinstance(dataset, IterableDataset):
        ...
        if has_len:
            if torch_distributed_enabled:
                ...
                if do_shuffle:
                    build_context.services.event.register_on_epoch_begin_event_listener(lambda epoch, is_train: sampler.set_epoch(epoch))
            else:
                if do_shuffle:
        ...
        data_loader = DataLoader(dataset, sampler=sampler, batch_size=batch_size, drop_last=drop_last, worker_init_fn=worker_init_fn,
                                 num_workers=num_workers, collate_fn=collate_fn, pin_memory=pin_memory)
