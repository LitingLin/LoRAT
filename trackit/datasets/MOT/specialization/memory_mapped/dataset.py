...
class MultipleObjectTrackingDatasetTrack_MemoryMapped:
    def __init__(self, object_id, object_attribute,
                 full_sequence_image_size_matrix: np.ndarray,
                 object_frame_index_vector: np.ndarray,
                 object_bounding_box_matrix: np.ndarray,
                 object_bounding_box_validity_flag_vector: np.ndarray,
                 sequence_additional_attributes: LazyAttributesLoader):
        self.object_id = object_id
        self.object_attribute = object_attribute
        self.full_sequence_image_size_matrix = full_sequence_image_size_matrix
    ...
    def get_all_frame_sizes(self):
        if self.full_sequence_image_size_matrix.ndim == 1:
            return self.full_sequence_image_size_matrix[None, :].repeat(len(self.object_frame_index_vector), axis=0)
        return self.full_sequence_image_size_matrix[self.object_frame_index_vector, :]
...
class MultipleObjectTrackingDatasetSequence_MemoryMapped:
    ...
    def get_all_frame_sizes(self):
        ...
        return self.sequence_image_size_matrix
    ...
    def get_object_by_id(self, object_id: int):
        ...
        if self.sequence_image_size_matrix is None:
            full_sequence_image_size_matrix = self.sequence_attributes['frame_size']
        else:
            full_sequence_image_size_matrix = self.sequence_image_size_matrix

        return MultipleObjectTrackingDatasetTrack_MemoryMapped(object_id, object_attribute,
                                                               full_sequence_image_size_matrix,
...
class MultipleObjectTrackingDataset_MemoryMapped(MemoryMappedDataset):
    ...
    def load(root_path: str, path: str):
        return MultipleObjectTrackingDataset_MemoryMapped(root_path, MemoryMappedDataset._load_storage(path))
