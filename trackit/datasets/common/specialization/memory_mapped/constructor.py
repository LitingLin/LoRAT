import numpy as np
from trackit.datasets.common.specialization.memory_mapped.engine import ListMemoryMappedConstructor
...
def memory_mapped_constructor_common_preliminary_works(base_dataset: dict, base_dataset_type: str, path: str,
    ...
    if 'context' in base_dataset:
        ...
        if bounding_box_data_type == 'int':
            bounding_box_data_type = np.int64
        elif bounding_box_data_type == 'float':
            bounding_box_data_type = np.float64
        else:
...
def memory_mapped_constructor_generate_bounding_box_matrix(bounding_box_matrix, bounding_box_dtype):
    if all([bounding_box is None for bounding_box in bounding_box_matrix]):
        ...
        if bounding_box_dtype == np.int64:
            bounding_box_matrix_ = [[-1, -1, -1, -1] if bounding_box is None else bounding_box for
                                    bounding_box in bounding_box_matrix]
        else:
            bounding_box_matrix_ = [[float('nan'), float('nan'), float('nan'), float('nan')] if bounding_box is None else bounding_box for
                                    bounding_box in bounding_box_matrix]
    ...
    if bounding_box_matrix is not None:
        bounding_box_matrix = np.asarray(bounding_box_matrix, dtype=bounding_box_dtype)
    if bounding_box_validity_flag_vector is not None:
        bounding_box_validity_flag_vector = np.asarray(bounding_box_validity_flag_vector)
...
def memory_mapped_constructor_generate_object_category_id_vector(object_category_id_vector):
    if all(category_id is None for category_id in object_category_id_vector):
        return None
    object_category_id_vector = tuple(category_id if category_id is not None else -1 for category_id in object_category_id_vector)
    return np.asarray(object_category_id_vector, np.int64)
