...
from ...modules.mixformer_attn import MixFormerBlock
...
class LoRAT_DINOv2(nn.Module):
    def __init__(self, vit: DinoVisionTransformer,
        ...
        self.blocks = nn.ModuleList([MixFormerBlock.build_from_std_block(block) for block in vit.blocks])
        self.norm = vit.norm
    ...
    def _fusion(self, z_feat: torch.Tensor, x_feat: torch.Tensor):
        ...
        for i in range(len(self.blocks)):
            fusion_feat = self.blocks[i](fusion_feat, self.z_size[1], self.z_size[0], self.x_size[1], self.x_size[0])
