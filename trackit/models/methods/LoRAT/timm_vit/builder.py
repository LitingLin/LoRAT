...
def build_LoRAT_model(config: dict, model_impl_suggestions: ModelImplementationSuggestions):
    ...
    with model_impl_suggestions.device, set_default_dtype(model_impl_suggestions.dtype):
        if model_type == 'vit':
            from .lorat import LoRAT_timm_ViT
            model = LoRAT_timm_ViT(backbone, common_config['template_feat_size'],
                                         common_config['search_region_feat_size'])
        elif model_type == 'eva':
            from .lorat_eva import LoRAT_timm_EVA
            model = LoRAT_timm_EVA(backbone, common_config['template_feat_size'],
                                         common_config['search_region_feat_size'])
        else:
