...
FusedWindowProcessOp:Optional[torch.autograd.Function]=None
FusedWindowProcessReverseOp:Optional[torch.autograd.Function]=None
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix=None):
        ...
        if self.shift_size > 0:
            if not self.fused_window_process or x.device.type != "cuda":
                shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
                # partition windows
                x_windows = window_partition(shifted_x, self.window_size)  # nW*B, window_size, window_size, C
            else:
                x_windows = FusedWindowProcessOp.apply(x, B, H, W, C, -self.shift_size, self.window_size)
        ...
        if self.shift_size > 0:
            if not self.fused_window_process or x.device.type != "cuda":
                ...
                x = FusedWindowProcessReverseOp.apply(attn_windows, B, H, W, C, self.shift_size, self.window_size)
        else:
...
class BasicStage(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        attn_mask = self._get_attn_window(H, W, x.device)
    ...
    def _get_attn_window(self, H, W, device):
        name = f'attn_mask_{H}_{W}_{self.window_size}_{self.shift_size}'
        ...
        self.register_buffer(name, attn_mask, False)
        return attn_mask
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, out_indices=None, reshape=False):
...
_cfg = {
    ...
    'swin_base_patch4_window12_384_1k': dict(
...
def build_swin_transformer_backbone(name, load_pretrained=True, output_layers=(3,), frozen_stages=-1,
                                    overwrite_embed_dim=None, use_checkpoint=False, fused_window_process=True,
                                    **additional_params):
    ...
    if fused_window_process:
        try:
            from .window_process.window_process import WindowProcess, WindowProcessReverse
            global FusedWindowProcessOp, FusedWindowProcessReverseOp
            FusedWindowProcessOp = WindowProcess
            FusedWindowProcessReverseOp = WindowProcessReverse
        except ImportError:
            print("[Warning] Failed to import Fused window process")
            fused_window_process = False
    transformer = SwinTransformer(out_indices=output_layers, frozen_stages=frozen_stages, use_checkpoint=use_checkpoint,
                                  fused_window_process=fused_window_process,
